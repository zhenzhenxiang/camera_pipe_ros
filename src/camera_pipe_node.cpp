...
int main(int argc, char** argv)
{
  ...
  std::string path_IMGPipe =
      nh.param("camera_pipe_path", std::string("/tmp/_ap0102_pixel"));
  int cam_num = pnh.param("cam_num", 4);
  int image_width = pnh.param("image_width", 1280);
  int image_height = pnh.param("image_height", 960);

  std::string path_birdview_XMap_seg =
      pnh.param("birdview_XMap_path_seg", std::string("XMap_seg.yaml"));
  std::string path_birdview_YMap_seg =
      pnh.param("birdview_YMap_path_seg", std::string("YMap_seg.yaml"));

  std::string path_birdview_XYMap_park =
  ...
  if (cp.setMapPathForSeg(path_birdview_XMap_seg, path_birdview_YMap_seg) &&
      ...
      cp.loadCameraInfo(path_cam_info_front, "front_cam"))
  {
